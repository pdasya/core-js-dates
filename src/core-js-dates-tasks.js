...
function isDateInPeriod(date, period) {
  ...
  return checkDate >= startPeriodDate && checkDate <= endPeriodDate;
}
 ...
 * Date(2024, 1, 23) => 8
 */
function getWeekNumberByDate(inputDate) {
  let result = 0;
  const startWeek = new Date(inputDate.getFullYear(), 0, 1);
  while (startWeek.getDay() !== 1) {
    if (!result) result = 1;
    startWeek.setDate(startWeek.getDate() + 1);
  }

  if (inputDate < startWeek) return 1;

  while (inputDate >= startWeek) {
    result += 1;
 ...
 */
function getNextFridayThe13th(inputDate) {
  const result = new Date(inputDate);
  result.setDate(13);

  if (result > inputDate && result.getDay() === 5) return result;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const result = [];
  const currentDate = new Date(period.start.split('-').reverse().toString());
  const finishDate = new Date(period.end.split('-').reverse().toString());
  let workday = 0;
  while (currentDate <= finishDate) {
    while (workday < countWorkDays && currentDate <= finishDate) {
